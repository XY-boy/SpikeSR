...
from model_archs.dcn import DeformableConv2d
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, heads, qk_dim):
        ...
        self.dcn = DeformableConv2d(dim, dim, kernel_size=3, stride=1, padding=1, bias=True)

    def forward(self, x, y=None):
        """Forward function.
        If 'y' is None, it performs self-attention; Otherwise it performs cross-attention.
        Args:
            x (Tensor): Input feature，原始特征: 9*9 patchsize*patchsize=16*16 c=64
            y (Tensor): Support feature. Match 之后
        Returns:
        ...
        if y is None:
            ...
            x_deform = rearrange(x, 'b (h w) d -> b d h w', h=16, w=16)
            x_deform = self.dcn(x_deform)
            x_deform = rearrange(x_deform, 'b d h w -> b (h w) d')

            q, k, v = self.to_q(x_deform), self.to_k(y), self.to_v(y)  # DCN projection
...
class Match(nn.Module):
    ...
    def forward(self, x):
        b, n, c, h, w = x.size()  # 1 81 64 16 16
        avg_fea = x.mean(dim=(-2, -1))  # (b, n, c)  1 81 64
        q = self.norm(avg_fea)
        ...
        if self.training:
            ...
            indices = indices.flatten()  # (b*n,)=81
            v = x.flatten(0, 1)  # 81 64 ps ps
            y = v[indices]  # 4*25 64 16 16
            y = y.view(b, n, c, h, w)
            # print(777, y.size())
...
class GPA(nn.Module):
    '''
    Global pixel access from Coarse to Fine: Hierarchical Pixel Integration for Lightweight Image Super-Resolution,
    '''
    def __init__(self, dim=64, patch_size=16, mlp_dim=100, qk_dim=32):
    ...
    def forward(self, x):
        ps = self.patch_size
        step = ps - 2
        crop_x, nh, nw = patch_divide(x, step, ps)  # crop_x: (b, n, c, ps, ps)

        y = self.cross_match[0](crop_x)


        crop_x = self.blocks[0](crop_x, y)

        residual = patch_reverse(crop_x, x, step, ps)
        x = x + self.mid_convs[0](residual)
...
if __name__ == '__main__':
    input = torch.rand(2, 96, 128, 128).cuda()  # B C H W
    model = GPA(dim=96, patch_size=16).cuda()
